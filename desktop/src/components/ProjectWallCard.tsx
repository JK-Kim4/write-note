...
type Props = {
  ...
  index: number;
  onOpen: () => void;
  /** 다음 장면 한 줄 — blur/엔터로 변경 시에만 호출(값이 이전과 다를 때). */
  onSaveNextScene: (nextScene: string) => void;
  onDelete: () => void;
};

/**
 * 작업 벽 위 작품 한 장(핀으로 꽂힌 쪽지 느낌).
 * - 얼굴 = 마지막 문장(세리프, 가장 두드러지게) / 본문이 비면 빈 상태 안내
 * - 그 아래 = 작가가 직접 적는 "다음 장면" 한 줄(blur/엔터 저장)
 * - 날짜·요약·카운터는 두지 않는다(FR-003).
 */
