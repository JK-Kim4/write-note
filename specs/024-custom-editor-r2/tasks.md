...
- [ ] T001 베이스라인 게이트 GREEN 확인 — `cd frontend && pnpm exec vitest run src/components/custom-editor`(115) + `pnpm exec vitest run src/app/b`(page.test 7) + `pnpm exec tsc --noEmit`. 결과 기록(무회귀 비교 기준).
...
- [ ] T002 [P] `frontend/src/components/custom-editor/model.ts` — `MARK`(bold:1/italic:2/underline:4/strike:8) 상수 + `Mask`·`MarkRun={len,mask}` 타입 + `DocModel.markRuns: MarkRun[][]` 필드 추가(타입·상수 = TDD §5-5 완화). 빈 모델 INV-3에 `markRuns:[[]]` 포함.
- [ ] T003 `model.test.ts` — `blockRuns` 정규형 실패 테스트 작성(인접 동일 mask 병합·0길이 제거·len 합=블록 글자수) [RED].
- [ ] T004 `model.ts` — `blockRuns(model, blockIdx)` 구현 [GREEN].
- [ ] T005 `model.test.ts` — `toggleMark` 실패 테스트(부분/전체/구간경계/여러 run 횡단/재토글 해제) [RED].
- [ ] T006 `model.ts` — `toggleMark(model, lo, hi, mark)` 구현 + 결과 정규화 [GREEN].
- [ ] T007 `model.test.ts` — `marksAt` 실패 테스트(좌측 글자 기준·offset 0/블록시작 예외) [RED].
- [ ] T008 `model.ts` — `marksAt(model, offset)` 구현 [GREEN].
- [ ] T009 `model.test.ts` — `insertText`/`deleteRange` 마크 추종 실패 테스트(구간 늘어남·줄어듦·split·merge·삽입마스크) [RED].
- [ ] T010 `model.ts` — `insertText(model,lo,hi,text,mask)`/`deleteRange` markRuns 추종 + `reconcile`(blockAttrs+markRuns, INV-4·5) 구현 [GREEN]. **마크0 = 1라운드 동일** 보장.
- [ ] T011 `model.test.ts` — `splitBlock`/`mergeWithPrev`/`mergeWithNext`/`toggleHeading` markRuns 분할·이어붙임 실패 테스트 [RED].
- [ ] T012 `model.ts` — 위 4연산 markRuns 추종 구현(병합 시 두 블록 run-list 이어붙임 후 정규화) [GREEN].
- [ ] T013 [P] `measure.test.ts` — run 그룹핑·styled-span 구성 로직 실패 테스트(폭 mock; 픽셀은 CDP) [RED].
- [ ] T014 [P] `measure.ts` — `measureParagraphLines`/`measureLineXs`에 `marks: MarkRun[]` 인자 추가, run별 `<span>`(weight/style/decoration) 오프스크린 구성. **canvas 금지·마크0=1라운드 동일** [GREEN].
- [ ] T015 [P] `pmConvert.test.ts` — 마크 왕복 무손실(4종·복합)·idempotent(2회 왕복 동일)·미지원 마크 평탄화·**마크0 모델 1라운드 출력 바이트 동일** 실패 테스트 [RED].
- [ ] T016 [P] `pmConvert.ts` — `modelToPmJson`(blockRuns→text node `marks:[{type}]`)·`pmJsonToModel`(marks→비트마스크 환원·정규화) 구현. type=`bold`/`italic`/`underline`/`strike` [GREEN].
- [ ] T017 [P] `history.ts` — `Snapshot`에 `markRuns` 포함(pendingMarks 제외) + undo/redo 마크 복원 테스트.
...
- [ ] T018 [US1] `CustomEditor.tsx` `relayout` — 블록별 `blockRuns(model,i)`를 `measureParagraphLines`에 전달 + `ParsedBlock`에 `marks: MarkRun[]` 슬라이스 보유.
- [ ] T019 [US1] `CustomEditor.tsx` 렌더(`PageBox`) — 블록을 run별 styled `<span>`(font-weight/font-style/text-decoration)으로 렌더(측정 div와 동일 스타일 → 픽셀 일치).
- [ ] T020 [US1] `CustomEditor.tsx` `caretToScreen`/`screenToCaret`/`selectionRects` — `measureLineXs`에 해당 블록 `marks` 전달(혼합 폭 반영). affinity는 본 단계 1라운드 `<=` 유지(US3에서 대체).
- [ ] T021 [US1] `CustomEditor.tsx` — 선택 있을 때 Cmd+B/I/U(+툴바 4버튼) → `toggleMark(model, lo, hi, mark)`. **`onKey` 최상단 `if(e.isComposing||composingRef.current) return` 가드 아래** 배치(IME 회귀룰).
- [ ] T022 [US1] `CustomEditor.tsx` — 마크 구간 한가운데 입력 시 `marksAt` 좌측 상속으로 삽입(보류 마크는 US2). 평문 paste = 마스크 0.
- [ ] T023 [US1] 게이트 — `pnpm exec tsc --noEmit` + `pnpm exec vitest run src/components/custom-editor` + 무회귀 `pnpm exec vitest run src/app/b`(7 GREEN).
- [ ] T024 [US1] **dogfooding(사용자 게이트)** — quickstart §4 US1: 드래그 굵게·italic/underline/strike·혼합 wrap 캐럿 드리프트 0·저장 재로드 무손실·삽입삭제 마크 추종. IME 4케이스(조합 중 단축키).
