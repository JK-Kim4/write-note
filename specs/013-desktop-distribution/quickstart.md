...
#    - macos-latest → Narae-Note.dmg (universal, ad-hoc)
#    - windows-latest → Narae-Note-Setup.exe (NSIS)
...
- [ ] Release에 `Narae-Note.dmg` + `Narae-Note-Setup.exe` 자동 업로드(수동 0회)
- [ ] 한쪽 OS job을 일부러 실패시켜도 다른 OS 자산은 정상 게시되는지(`fail-fast:false`)

### G2. macOS 실제 설치 ⚠️ 최대 리스크 (R3 / FR-006/007)
- [ ] **빌드한 Mac이 아닌 다른 Mac**(또는 `xattr -w com.apple.quarantine` 부여)에서 `Narae-Note.dmg` 다운로드
