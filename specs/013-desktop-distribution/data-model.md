...
| OS | 파일명(고정) | 형식 | 특성 |
|---|---|---|---|
| macOS | `Soseolbi-Note.dmg` | dmg, universal(x64+arm64) | ad-hoc 서명(`identity:"-"`), 비공증 |
| Windows | `Soseolbi-Note-Setup.exe` | NSIS | 1-click, per-user(관리자 권한 불필요), 무서명 |

- 최신 링크: `https://github.com/JK-Kim4/write-note/releases/latest/download/<파일명>`
- `productName`: `Soseolbi` (`electron-builder.yml` 기존 값 유지), `appId`: `com.soseolbi.desktop`
...
| Windows 다운로드 버튼 | 고정 링크(`...releases/latest/download/Soseolbi-Note-Setup.exe`) | 항상 노출 |
| macOS 다운로드 버튼 | 고정 링크(`...releases/latest/download/Soseolbi-Note.dmg`) | 항상 노출 |
...
desktop/package.json version 상향
  → git tag v<semver> + push
    → [GitHub Actions] macos-latest: build → ad-hoc 서명 → Soseolbi-Note.dmg
    → [GitHub Actions] windows-latest: build → Soseolbi-Note-Setup.exe
