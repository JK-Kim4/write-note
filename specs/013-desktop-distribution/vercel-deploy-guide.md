...
이 가이드를 끝내면 `https://<당신의-vercel-주소>/` 에서 소설비 다운로드 페이지가 인터넷에 공개됩니다.
...
→ "소설비 다운로드" 화면 + Windows/Mac 버튼 + 설치 안내문이 보이고, 방문 OS 버튼이 강조되며, 버튼을 누르면 GitHub Releases에서 설치파일이 받아지면 성공입니다.
...
- 다운로드 대상: `https://github.com/JK-Kim4/write-note/releases/latest/download/Soseolbi-Note.dmg` · `…/Soseolbi-Note-Setup.exe`
