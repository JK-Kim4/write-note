        ...
        <div className="rounded-xl border border-border bg-surface">
            ...
            >
                <div className="min-w-0">
                    <h2 className="truncate text-base font-bold text-ink">{card.project.title}</h2>
                    <p className="mt-0.5 text-xs text-faint">
                        {card.wordCount.toLocaleString()}자 · 누적 {formatDurationKo(card.totalDurationMs)}
                    </p>
                    {sentence && (
                        <p className="mt-1 truncate text-sm text-muted-strong">마지막 문장 — {sentence}</p>
                    )}
                    {card.latestLog && (
                        <p className="mt-0.5 truncate text-sm text-muted sm:hidden">{card.latestLog.body}</p>
                    )}
                </div>
                <div className="flex shrink-0 items-center gap-3">
                    {card.latestLog && (
                        <span className="hidden max-w-64 truncate text-sm text-muted sm:block">
                            {card.latestLog.body}
                        </span>
                    )}
            ...
            {isOpen && (
                <div id={panelId} className="border-t border-border px-4 py-3">
                    {logsQuery.isLoading ? (
                        ...
                        <div>
                            <p className="text-xs text-muted">기록을 불러올 수 없습니다.</p>
                            <button
                                type="button"
                                onClick={() => logsQuery.refetch()}
                                className="mt-2 rounded-md border border-border-strong px-3 py-1 text-xs text-muted-strong hover:bg-surface-2"
                            >
                                다시 시도
                            </button>
                        </div>
                        ...
                        <ul className="space-y-2">
                            {logs.map((log) => (
                                <li key={log.id} className="flex items-baseline gap-3">
                                    <span className="shrink-0 text-xs text-faint">{formatDateTime(log.createdAt)}</span>
                                    <span className="text-sm whitespace-pre-wrap text-ink-2">{log.body}</span>
                                </li>
                            ))}
                        </ul>
                    )}
        ...
        <div>
            ...
            {isLoading ? (
                ...
                <div className="py-12 text-center">
                    <p className="text-sm text-muted">기록을 불러올 수 없습니다.</p>
                    <button
                        type="button"
                        onClick={() => refetch()}
                ...
                <>
                    ...
                    <div className="space-y-3">
                        ...
                        {(cards ?? []).length === 0 && (
                            <p className="py-8 text-center text-sm text-muted">아직 작품이 없습니다.</p>
                        )}
                    </div>
                </>
            )}
