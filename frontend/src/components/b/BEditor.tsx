...
/**
 * B타입 본문 에디터 — fable-test ChapterEditor 디자인 이식 (툴바 / 본문 / 상태바 세로 3단).
 *
 * 페이지 분할 없는 흐름형 본문(max-width 48rem). 자동저장·draft 결선은 A 디자인 PaperEditor 와
 * 동일 규약: 한국어 IME 조합 중에는 부모 state 갱신을 억제하고(onChange 차단) draft(onDraftUpdate)는
 * 항상 흘려보낸다(작성분 무유실, PoC 0-1 가드).
 */
...
export function BEditor({ initialBodyJson, onChange, onDraftUpdate, onEditorReady, statusLabel, statusTone }: BEditorProps) {
    // 초기 글자수는 본문 JSON 에서 직접 파생(effect 내 setState 회피) — 이후엔 onUpdate 가 갱신.
    ...
    return (
        <div className="flex h-full min-w-0 flex-1 flex-col overflow-hidden rounded-xl border border-gray-200 bg-white">
            ...
            {/* editor-scroll — useEditorOutline 의 현재 섹션 추적이 이 클래스를 스크롤 컨테이너로 찾는다. */}
            <div className="editor-scroll b-editor flex-1 overflow-y-auto px-8 py-6">
