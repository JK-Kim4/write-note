...
describe("OnboardingTour v2 — 단계 구성", () => {
    it("총 6 단계(인트로 3 + 메뉴 3)를 driver 에 전달한다", async () => {
        server.use(settingsResponse({}));
        renderTour();
        await waitFor(() => expect(driveMock).toHaveBeenCalled());
        // steps 배열 길이가 6
    ...
    it("메뉴 3 단계는 nav-works / nav-memos / nav-characters 순서로 타겟을 가진다", async () => {
        server.use(settingsResponse({}));
        renderTour();
        await waitFor(() => expect(driveMock).toHaveBeenCalled());
        const steps = capturedConfig?.steps as Array<{ element?: string; popover?: unknown }>;
        expect(steps[3].element).toBe('[data-tour="nav-works"]');
        expect(steps[4].element).toBe('[data-tour="nav-memos"]');
        expect(steps[5].element).toBe('[data-tour="nav-characters"]');
    });
});
...
describe("OnboardingTour v2 — 분기", () => {
    ...
    it("step 6 은 [바로 시작]/[더 보기] 2개 버튼을 명시 렌더한다(× 의존 아님)", async () => {
        server.use(settingsResponse({}));
        renderTour();
        await waitFor(() => expect(driveMock).toHaveBeenCalled());
    ...
    it('"바로 시작" 버튼(step 6 previous) 클릭 시 완료 저장 후 router.push 호출하지 않는다', async () => {
        ...
        renderTour();
        await waitFor(() => expect(driveMock).toHaveBeenCalled());
        // "바로 시작" = step 6 의 previous 슬롯 핸들러(뒤로 가지 않고 완료+종료)
    ...
    it('"더 보기" 선택 시 완료 저장 + sessionStorage 핸드오프 set + router.push("/library") 호출', async () => {
        ...
        renderTour();
        await waitFor(() => expect(driveMock).toHaveBeenCalled());
        // "더 보기" 내부 handler 는 OnboardingTour.tsx 가 step 6의 onNextClick 에 주입
        // capturedConfig 에서 꺼내 발화
        const steps = capturedConfig?.steps as Array<{
            popover?: { onNextClick?: () => void };
        }>;
        // step 6(index 5)의 onNextClick 이 "더 보기" 핸들러
