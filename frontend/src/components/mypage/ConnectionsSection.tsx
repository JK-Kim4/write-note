...
export function ConnectionsSection({ kakaoLinked, passwordSet }: { kakaoLinked: boolean; passwordSet: boolean }) {
    ...
    return (
        <section className="rounded-xl border border-gray-200 bg-white p-5">
            <h2 className="text-base font-semibold text-gray-900">계정 연결</h2>
            <p className="mt-0.5 text-xs text-gray-400">로그인 수단을 추가로 연결할 수 있어요. 연결 해제는 지원하지 않습니다.</p>

            <dl className="mt-4 space-y-4">
                {/* 이메일 / 비밀번호 */}
                <div className="flex flex-col gap-2 border-b border-gray-100 pb-4">
                    <div className="flex items-center justify-between">
                        <dt className="text-sm font-medium text-gray-700">이메일 · 비밀번호</dt>
                        <dd className={passwordSet ? "text-sm text-teal-600" : "text-sm text-gray-400"}>
                            {passwordSet ? "연결됨" : "미설정"}
                        </dd>
                    </div>
                    {!passwordSet ? (
                ...
                {/* 카카오 */}
                <div className="flex items-center justify-between">
                    <dt className="text-sm font-medium text-gray-700">카카오</dt>
                    <dd>
                        {kakaoLinked ? (
