...
4. **공유(비격리) 테스트 DB 주의** — 테스트가 격리되지 않은 공유 DB(로컬 PG 등)에 붙으면 **누적 픽스처가 잠복 회귀를 가린다**. 게이트 GREEN 은 잠복 버그 부재의 증거가 아니다. 의심(특히 subagent 의 "기존·무관" 자기진단) 시 **격리/fresh 픽스처로 재실행**(`--rerun-tasks` / 클린 DB)해 직접 관찰한다.
...
### 회귀 사례 — 2026-06-23 034 본문 암호화 readOnly-create 잠복 버그 (공유 DB가 가림)

- 수정 subagent 가 9개 테스트 실패를 "기존 베이스에도 있던 실패 / 내 변경 무관" 으로 보고. 그러나 직전 베이스가 GREEN 이었음(정면 모순).
- advisor 가 `./gradlew test --rerun-tasks` 로 fresh 픽스처 강제 → 9개 실패 직접 관찰 → 단독 실행에서도 재현(코드 버그 확정) → 서버 로그 `ERROR: cannot execute INSERT in a read-only transaction` 로 근본원인 직격: `BodyCipherService.decryptToPlain` 이 복호 시에도 `getOrCreate` 로 DEK INSERT 시도 → readOnly 트랜잭션(문서 로드·카드) 에서 500. 복호-비생성(레거시 통과 / 기존 DEK 만 조회) 으로 수정 후 GREEN.
- **공유 로컬 PG 의 누적 DEK 픽스처행이 버그를 가렸고**, `--rerun-tasks` 가 만든 fresh(unique email) 픽스처에서만 노출됨. 게이트 GREEN(`UP-TO-DATE` 캐시 포함)을 잠복 부재 증거로 단정했다면 회귀 merge 위험.
- 회피 가능했던 시점: subagent "기존·무관" 보고 직후 `--rerun-tasks` 재실행(본 사례는 차단함 — §7-4 로 영구화).
...
4. **버그 보고 = 실재 확인부터** — 버그 보고·스크린샷·로그 단편을 받으면 **"이 버그가 실재하는가"를 재현/사용자 확인으로 먼저 확정**한 뒤 근본원인을 단정한다(systematic-debugging Phase 1 정합). 단편만으로 "CRITICAL 버그" 단정 + 수정안 제시 금지. **또한 피드백 스크린샷·보고는 "동작 실패"인지 "동작은 되나 모습/의도가 다름"인지 먼저 분기**한다 — 한쪽으로 단정해 수정·질문(특히 단정형 선택지)을 송출하기 전에 사용자가 가리키는 것이 무엇인지 열린 확인부터. 회귀: 2026-06-27 044 — 빈 보드 안내 스크린샷을 "안내 미표시(빈 카드 잔존 부작용)"로 단정해 `AskUserQuestion` 송출 → 사용자 거부, 실제는 "안내가 뜨긴 하나 모습(전체 흰 화면)이 의도와 다름".
...
## 22. 운영/인프라 상태를 검증 없이 단정 금지 (HARD-GATE)

로컬에서 관측된 결함의 **운영 영향**, **운영 배포 상태**, **운영 데이터 상태**를 "아마 정상 / 이미 됐을 것"으로 단정하지 않는다. 운영 상태를 답변에 포함하기 전 **읽기 전용 조회로 확정**한다. 결론이 우연히 맞더라도 검증 안 된 이유는 추측이다(CLAUDE.md 금지2 연장).

### 회피 절차

1. 운영 영향·상태를 말하기 직전: "이게 검증된 사실인가, 추측인가" self-check.
2. 검증 가능하면 읽기 전용 조회 — 운영 DB(Flyway 버전·테이블 존재·행 상태), 컨테이너 env 존재 여부(값 미노출), 배포 버전.
3. 검증 불가면 "미검증 추정"으로 명시. 단정 금지.
4. **배포(특히 main 승격) 전 실제 배포 범위(누적 커밋·새 마이그레이션)를 `git log main..develop` 로 확정** — 무엇이 누적/미배포인지 메모리로 단정 말 것. 메모리의 배포 상태 정보는 그 사이 배포로 stale 할 수 있다.

### 회귀 사례 — 2026-06-24 034 DEK 결함 운영 영향 단정

- 로컬 DEK 미존재 결함(암호문↔DEK 불일치) 발견 후 "운영은 DEK 가 있어 정상"이라 **검증 없이 단정**. 사용자가 "로컬 문제면 운영도?"로 의심 → 운영 DB 직접 조회 결과 034 미배포(V21, `user_encryption_keys` 부재)라 본문이 평문이라 안전. **결론(운영 안전)은 맞았으나 이유("DEK 있어서")가 틀린 추측**.
- 회피 가능 시점: "운영 정상" 발언 전 운영 DB Flyway 버전 조회 1회.

### 회귀 사례 — 2026-06-24 037 main 승격 배포 범위 추측

- main 승격 직전 "develop 에 034 등 미배포 누적분이 한꺼번에 production 으로 올라간다"고 우려 표명 → `git log main..develop` 확인 결과 main 이 이미 034 포함(격차 = 036+037+docs 만, 새 마이그레이션 V23 1개). **결론은 맞았으나 추측이 격차 확인보다 한 박자 앞섬**. 메모리 [[deployment-live]]의 "034 미배포" 가 그 사이 배포로 stale 했음.
- 회피 가능 시점: 배포 범위 우려를 말하기 전 `git log main..develop` 1회.

## 23. 형상관리되는 운영 자산은 repo 정본을 먼저 본다 (HARD-GATE)

배포 스크립트·Dockerfile·설정 템플릿 등 **형상관리될 수 있는 운영 자산**의 구조·동작을 파악할 때, 운영 서버(ssh/grep)보다 **repo 의 정본을 먼저 확인**한다. 서버 사본은 배포 결과일 뿐 SoT 가 아니며, 서버 직접 조회는 시크릿 노출 위험도 더 크다.

### 회피 절차

1. 운영 자산(스크립트/Dockerfile/설정)의 동작 확인 필요 시: repo 에 형상관리되는지 `find`/`grep` 먼저.
2. repo 에 있으면 정본을 Read(시크릿 없음). 서버 사본은 배포 검증용으로만.
3. repo 에 없는 서버 전용 파일(시크릿 env 등)만 서버 조회, 그때도 값 미노출.

### 회귀 사례 — 2026-06-24 배포 스크립트 OCI grep

- KEK 주입 방식을 OCI 에 `ssh` `grep` 으로 확인. 사용자가 "배포 스크립트는 형상관리 파일 아냐?" 지적 → 실제 `backend/deploy/blue-green-deploy.sh` 로 형상관리되는 자산. repo 정본을 먼저 봤어야(서버 사본은 그 배포물).
- 회피 가능 시점: 인프라 자산 확인 시 repo `find` 먼저.

## 24. structured 입력 도구는 structured 파라미터로 호출 — raw JSON 수동 직렬화 금지 (HARD-GATE)

`AskUserQuestion` 등 구조화 입력 도구는 **제공된 구조화 파라미터**(`questions` 배열 등)로 호출한다. 한글 등 비라틴·이스케이프가 많은 입력을 raw JSON(`__unparsedToolInput` 등)으로 수동 직렬화하면 백슬래시·유니코드 이스케이프가 깨져 파싱 실패한다. **첫 호출부터** 구조화 파라미터를 쓴다.

### 회피 절차

1. 구조화 입력 도구 호출 시 raw/unparsed JSON 필드를 손으로 채우지 않는다.
2. 도구가 노출한 구조화 파라미터(배열·객체)에 값을 직접 넣는다 — 직렬화는 런타임에 맡긴다.

### 회귀 사례 — 2026-06-25 039 트랙 A (+ 2026-06-25 038 재발)

- 039 트랙 A 세션에서 `AskUserQuestion` 한글 옵션을 raw JSON 수동 직렬화로 보내다 **4회 파싱 실패**(매번 두 번째 시도에서 structured 파라미터로 정상화 — 순수 낭비).
- **038 플롯 보드 회고 §5-2 후보**로 이미 도출됐으나 룰 미승격 → 같은 세션군에서 재발. 2회 누적이라 HARD-GATE 로 승격.
- 회피 가능 시점: 첫 `AskUserQuestion` 호출부터 structured `questions` 배열.

## 25. dogfooding "통과" 단정은 명시 체크리스트 전항을 사용자가 확인한 뒤에만 (HARD-GATE, verification-before-completion 연장)

dogfooding/수동 검증 게이트는 **일부 항목 통과를 전체 통과로 단정하지 않는다.** 마무리(커밋·배포·서버 정리·finish-work) 직전, 제시한 체크리스트의 **전항을 사용자가 확인했는지** 명시 확인한다. 일부만 받고 마무리로 직행하면 사용자 정정 + 환경 재기동 왕복이 생긴다.

### 회피 절차

1. dogfooding 체크리스트를 제시했으면, 사용자 응답이 **그 전항을 덮는지** 대조한다(일부 항목 언급 ≠ 전항 통과).
2. 누락 항목이 있으면 "나머지 N개 미확인"으로 명시하고 그것부터 확인 요청 — 마무리 게이트(서버 중지 등)로 직행 금지.
3. 마무리 직전 "체크리스트 전항이 통과됐는가"를 self-check.

### 회귀 사례 — 2026-06-25 039 트랙 A
...
## 29. UI/UX·인터랙션 결정은 글/메뉴보다 인터랙티브 목업 우선 (HARD-GATE)

화면 배치·컴포넌트 동작·열고닫기/애니메이션/hover/드래그 같은 **UI/UX·인터랙션 결정**을 사용자에게 제시할 때, 긴 산문 설명이나 텍스트 선택지 메뉴(`AskUserQuestion`)보다 **실제 앱 색·치수의 인터랙티브 목업**(`docs/research/YYYY-MM-DD-*-mockup.html`, `open` 으로 브라우저)을 **첫 결정부터** 우선한다. 세부 동작은 직접 합리적 기본값으로 판단해 목업에 반영하고, 진짜 양자택일만 시각 옵션으로 가른다. 인터랙션(열고닫기·토글·슬라이드)이면 정적 비교가 아니라 **실제로 클릭·동작하는** 목업으로 만든다. 백엔드·순수 로직 등 시각/인터랙션 표면이 없는 결정엔 적용 안 함(그건 글/표). 구체 절차는 스킬 `ux-mockup`.

### 회피 절차

1. UX/인터랙션 결정 지점 인지 → "이건 봐야 안다" 싶으면 산문 설명·메뉴 송출 **전** 목업 모드로 전환.
2. 앱 토큰(`tokens.css`)·기존 컴포넌트·이전 목업에서 색/치수 차용(추측 금지) → 실제처럼.
3. 합의될 때까지 같은 목업 파일 iterate. 확정 결정은 설계문서/spec에 박기(대화에만 두지 않음).

### 회귀 사례 — 2026-06-27 045/046 보드 코치마크·인라인 오버레이

- 코치마크 위치·집필 보드 편집 방법·오버레이 열고닫기 **세 UX 결정에서 각각 글 설명/`AskUserQuestion` 메뉴로 먼저** 감 → 사용자 3회 정정("글로는 이해가 안된다 목업", "목업으로", "글로 설명하면 이해가 어려우니까 알아서 목업으로 제시 + 스킬로 만들어"). 세 번째에야 `ux-mockup` 스킬로 영구화.
- 회피 가능 시점: 첫 UX 결정(코치마크 위치)에서 즉시 인터랙티브 목업(브레인스토밍 Visual Companion 정합).

## 30. cwd 누적·공유 환경에서 중요 명령은 절대경로/subshell 격리 (HARD-GATE, 룰20 연장)

같은 셸 세션에서 `cd` 가 누적되거나 병렬 Bash 호출이 작업 디렉토리(cwd)를 공유하면, **상대경로가 직전 `cd` 잔여를 따라 깨진다**. 배포·전송·빌드 등 **결과가 중요한 명령은 절대경로 또는 `(cd /abs && ...)` subshell 로 격리**한다. 또한 `cmd | tail` 처럼 파이프로 묶어 **exit code 를 가리지 않는다** — 실패가 가려진 채 후속 명령(배포 스크립트 등)이 무효 실행되면 "처리됐겠지"로 오인한다(룰21 §3 출력 미확인 함정의 cwd 판).

### 회피 절차

1. 배포 jar `scp`·`bootJar`·게이트 빌드 등 중요 명령은 절대경로 또는 `(cd /abs && ...)` subshell 로 작성.
2. 실패를 봐야 하는 명령은 `| tail` 로 묶지 말거나, 별도로 exit code(`echo "exit=$?"`)를 확인한 뒤 다음 단계로.

### 회귀 사례 — 2026-06-27 보드 배포 scp cwd race

- BE 배포 1차 `scp build/libs/...`(상대경로)가 직전 `cd backend` 잔여로 cwd 가 backend → `backend/backend/build/...` 로 깨져 `No such file or directory`. `scp ... | tail` 이 exit code 를 가려 deploy 스크립트가 그대로 실행 → **이전 jar 로 재배포(Docker `COPY ... CACHED`) → 운영 Flyway V23 머묾**(새 코드·마이그레이션 미적용). 운영 health 200 유지(무중단). 절대경로 재scp+재배포로 V26 적용. **같은 cwd race 가 FE 게이트 첫 시도(병렬 Bash cwd 공유)에서도 발생** — 한 세션 2회. 회피 가능 시점: 명령 작성 시 절대경로/subshell + 파이프로 exit code 가리지 않기.

## 31. 서브에이전트의 외부 인프라 상태 '추정'은 사용자 안내·결정 반영 전 직접 조회로 확정 (HARD-GATE, §7·§22 연장)

서브에이전트가 외부 인프라(클라우드 프로젝트 존재 / 배포 상태 / 설정)를 **'추정' 또는 '대시보드에만 기록' 류로 보고**하면, 그 추정을 **사용자 안내·삭제 지시·결정에 반영하기 전 read-only 조회로 확정**한다(`vercel projects ls`, 클라우드 콘솔 조회 등). 추정을 사용자 작업 지시로 송출하면 헛 안내가 된다. §22(운영 상태 검증 없이 단정 금지)의 외부 인프라 판 + §7(서브에이전트 자기진단 무검증 수용 금지)의 인프라 확장.

### 회피 절차

1. 서브에이전트의 외부 인프라 존재/상태 보고가 "추정/대시보드에만 기록" 류면, 사용자 안내·결정에 반영하기 전 read-only 조회로 확정.
2. 확정 불가면 "미확인 추정"으로 명시 — 사용자 작업 지시(삭제 등)로 단정 송출 금지.

### 회귀 사례 — 2026-06-27 download-site Vercel 부존재

- 폐기 데스크톱 정리 중 Explore 서브에이전트가 "download-site = 별도 Vercel 프로젝트(대시보드에만 기록)"라 **추정** → 검증 없이 수용 → 사용자에게 "Vercel 대시보드에서 삭제하세요"라고 사용자 영역 작업으로 안내. 실제로는 `vercel projects ls` 결과 `narae-note` 팀에 `write-note`·`soseolbi-admin` 2개뿐, download-site 프로젝트 **실재하지 않음**. 사용자 "vercel cli 연결되어있지않아?" 질문으로 조회 → 정정 + 메모리 [[deployment-live]] 정정. 회피 가능 시점: 사용자 안내 송출 전 `vercel projects ls` 1회.

## 32. 두 레이어가 공유하는 구조·위치 모델은 정본(canonical) 한쪽을 정하고 설계 시점에 양쪽 정합 검증 (HARD-GATE, §12·§28 연장)

FE·BE 등 두 레이어가 같은 **위치/구조 모델**(블록 인덱스·문자 오프셋·정렬 키·트리 평탄화 순서 등)을 **독립적으로 계산**하면, 어느 쪽이 정본(SoT)인지 설계(data-model/contracts) 시점에 정하고 다른 쪽이 그걸 **미러링하는지 그때 검증**한다. 한쪽 문서·구현만 보고 모델을 정의하면 다른 쪽과 어긋나 런타임에 **거짓 거부(검증 400)·오정렬**이 난다 — 단위 게이트는 한 레이어만 보므로 못 잡고, 통합/리뷰/dogfooding 에서야 표면화한다.

### 회피 절차

1. 공유 위치/구조 모델 식별 — "FE 와 BE 가 같은 인덱스/오프셋/순서를 각자 계산하는가?"
2. 정본 결정 — 보통 **사용자가 보고 상호작용하는 렌더 결과**가 SoT(검증·저장은 그걸 따라야 함).
3. 비정본 레이어가 정본의 산출을 **동형으로 재현**하는지 설계 시점에 대조(평탄화 규칙·경계·특수문자 길이까지). 언어가 달라 코드 재사용 불가면 규칙을 1:1 포팅하고 교차 케이스(목록·중첩·특수문자) 테스트로 잠근다.

### 회귀 사례 — 2026-06-28 046 공유 댓글 앵커 BE↔FE 불일치

- 설계 문서(`research.md` R-4)가 댓글 앵커를 "PM **top-level 블록**"이라 정의 → BE `AnchorValidator` 가 그대로 top-level 노드를 셈. 그러나 FE `pmConvert` 는 목록=항목별·인용=단락별·hardBreak=1글자로 평탄화(렌더가 그걸 그림). 두 모델이 어긋나 **목록/인용/소프트줄바꿈 본문에서 정상 댓글이 거짓 400(COMMENT_ANCHOR_INVALID)** 으로 거부됨. 서브에이전트 보안 리뷰가 차단, BE 를 FE 평탄화에 동형으로 재작성해 화해. 회피 가능 시점: data-model/research 에 앵커 "블록"을 적을 때 FE `pmConvert` 평탄화를 확인해 "렌더 평탄화 블록"으로 정의했어야.

## 33. 비ASCII·비가시 제어문자는 소스에 리터럴 대신 escape 시퀀스로 기입 (HARD-GATE, §24 계열 — 도구 입출력 hygiene)

U+2028(LINE SEPARATOR) 등 **비ASCII·비가시 제어문자**를 소스 코드에 넣을 때, 리터럴 문자 대신 언어의 escape(`\uXXXX` 형식)를 쓴다. 파일 기입 도구(Write/Edit)가 리터럴을 정규화·치환해 **다른 코드포인트(예: 일반 공백)로 망가뜨릴 수 있고**, 화면·diff 에서 구분이 안 돼 조용히 틀린다. 불가피하게 리터럴이 됐으면 `od -An -tx1`(또는 hexdump)로 바이트를 확인한다.

### 회피 절차

1. 소스에 넣을 문자가 비ASCII·비가시(제어/구분/zero-width)면 escape 시퀀스로 작성.
2. 도구가 망가뜨린 게 의심되면 바이트 확인(`sed -n 'Np' file | od -An -tx1`). U+2028 = `e2 80 a8`, space = `20`.
3. 결정적 재기입이 필요하면 escape 를 명시 출력하는 경로(예: `python3` heredoc 에서 `"\\uXXXX"`)로 쓴다.

### 회귀 사례 — 2026-06-28 046 AnchorValidator SOFT_BREAK 망가짐

- `private const val SOFT_BREAK = "…"` 에 U+2028 리터럴을 Write 로 기입했으나 실제 바이트가 U+2028 이 아니게 망가짐(`od`/`grep` 으로 `e2 80 a8` 부재 확인). FE `model.ts` SOFT_BREAK 와 코드포인트 불일치 시 hardBreak 길이 계산이 어긋남. `python3` 으로 escape(`\u2028`) 결정적 재기입 + 바이트 검증으로 해소(~3 도구 호출 낭비). 회피 가능 시점: 처음부터 escape 시퀀스로 작성.

## 34. 사용자가 버그를 실증하는데 정적 분석이 깨끗하면 "정상" 아닌 "원인 미규명" 보고 (HARD-GATE, §16 연장 / CLAUDE.md 금지2)

사용자가 버그를 실증(영상·재현·실측)하는데 코드·빌드·배포·정적 산출물이 정상으로 보이면, "정상이다 / 버그 없다 / 코드 문제 아님"으로 단정하지 않는다. **정적 분석이 깨끗한 것은 버그 부재의 증거가 아니다 — 관찰된 증상이 우선한다.** 원인을 못 찾았으면 "아직 원인 미규명"으로 정직히 보고하고, 정적 분석이 못 잡는 영역(레이아웃·이벤트·런타임 상태·환경)을 의심해 실제 환경 재현(브라우저·prod 빌드)으로 전환한다. CLAUDE.md 금지2(단정 금지)·§16(고친 곳=관찰한 곳)의 디버깅 판.

### 회피 절차

1. 코드/빌드/배포/정적 산출물이 정상으로 보여도, 사용자가 실증한 버그가 있으면 "정상" 단정 금지.
2. "정적 분석은 깨끗하나 원인 미규명 — 실제 환경에서 재현해 관찰하겠다"로 보고.
3. 정적 분석(코드 정독·grep·단위테스트)이 못 잡는 영역을 의심해 실측/재현으로 전환. 특히 단위테스트(jsdom 등) 통과를 시각·레이아웃·이벤트 버그의 "정상" 증거로 쓰지 않는다.

### 회귀 사례 — 2026-06-28 공유 모달 ✕ 안 닫힘

- 사용자가 ✕ 안 닫힘을 영상으로 실증하는데, 코드 정독·prod 빌드·배포·운영 chunk 가 모두 정상으로 보이자 "버그 없다/정상이다/코드 문제 아님"으로 반복 단정 → 사용자 "안보여? 이게 정상이라고?" 격분. 실제 root cause 는 카드-팝오버 DOM 얽힘(레이아웃)이라 정적 분석이 못 잡는 영역이었고 사용자가 직접 진단. jsdom vitest 통과도 "코드 정상" 증거로 오인. 회피 가능 시점: 영상으로 버그가 명백한 순간 "정상"이 아니라 "원인 미규명"으로 보고 + 로컬 prod 재현으로 전환했어야.
- 회고: `~/obsidian/write-note/retrospectives/2026-06-28-share-modal-close-portal-debug.md`

## 35. 공유 값의 스코프·의미를 바꾸면 그 값을 소비·집계·파생하는 모든 레이어를 변경 시점에 전수 감사 (HARD-GATE, §32 연장)

한 레이어에서 공유되는 값의 **스코프/의미**(집계 단위·키·정렬 기준 등, 예: projectId 단위 → snapshot 단위)를 바꿀 때, 그 값을 읽어 **집계·파생·대표값 선택**하는 모든 소비처를 변경 시점에 전수 확인한다. 소비처가 "대표값 하나 취함(dedup/first)"이면 스코프 변경 후 **과소·과대 집계로 조용히 깨진다** — 단위 게이트·타입 검사가 못 잡는다(값의 타입은 그대로, 의미만 바뀌므로).

### 회피 절차

1. 공유 값의 스코프/의미를 바꾸기로 한 시점에 `grep <값 이름>`(예: `unreadCommentCount`)으로 소비처를 전수 나열.
2. 각 소비처가 그 값을 **합산/평균/대표값-취함(first·dedup)** 중 무엇으로 쓰는지 확인. 옛 스코프에선 맞던 "대표값-취함"이 새 스코프에선 틀릴 수 있다.
3. 소비처의 테스트가 옛 스코프 가정(예: "여러 링크 동일 값")을 인코딩했으면 함께 갱신.

### 회귀 사례 — 2026-07-02 050 공유 unread 배지 교차층

- BE `unreadCommentCount`를 projectId 스코프 → 스냅샷 스코프로 바꿨는데(읽음 처리와 정합 위해), FE `shareGrouping.unreadProjects`가 projectId별 **첫 스냅샷 값만 취하던(dedup)** 로직 그대로라 다중 링크 작품의 상단 "받은 피드백" 합계를 **과소집계**. 옛 스코프(각 스냅샷=projectId 총합)에선 대표값-취함이 맞았으나 새 스코프(스냅샷별 개별 값)에선 합산이어야. 게이트 통과·advisor가 후속 지적으로 발견.
- 회피 가능 시점: BE unread 스코프 변경 시 `grep unreadCommentCount`로 FE 소비처(shareGrouping) 전수 추적. 회고 `~/obsidian/write-note/retrospectives/2026-07-02-share-enhancements-050.md`.

## 36. 리뷰 후 적용한 수정은 그 자체가 미리뷰 — 고영향 수정은 적용 직후 재검 (HARD-GATE, §7 연장)

리뷰(서브에이전트·advisor·사람)는 **리뷰 시점 코드만** 본다. 리뷰 후 적용한 수정은 리뷰어가 못 본 새 코드이므로, 특히 **교차층·집계·스코프·공용 레이어를 건드리는 수정**은 적용 직후 그 수정 자체를 재검(소비처 추적 또는 재리뷰)한다. "리뷰 통과"를 그 뒤 수정본의 검증으로 오인하지 않는다.

### 회피 절차

1. 리뷰 발견을 수정한 뒤, 그 수정이 **새 표면(교차층·집계·공용)**을 건드렸는지 self-check.
2. 건드렸으면 그 수정에 대해 소비처 전수 추적(§35) 또는 짧은 재리뷰 1회. 원 리뷰가 이 수정을 못 봤음을 명시.
3. 여러 수정을 배치로 적용했으면 배치 전체를 한 번 더 훑는다(수정이 서로 간섭할 수 있음).

### 회귀 사례 — 2026-07-02 050 HIGH 수정의 FE 꼬리

- 리뷰 에이전트 3종이 **수정 전 코드**를 리뷰. HIGH(읽음 배지 스코프) 수정을 적용했는데 그 수정의 FE 교차층 꼬리(§35)는 리뷰어가 못 본 새 코드라 아무도 안 잡음 → advisor가 후속 지적. 내 fix 자체가 미리뷰 상태로 shipping 직전까지 감.
- 회피 가능 시점: HIGH 수정 적용 직후 "이 수정이 새로 건드린 소비처는?" 재검. 회고 위 경로.

## 37. 배포 상태를 설계 문서 여러 곳에 사실로 박기 전에도 git+메모리 교차확인 (HARD-GATE, §22 강화)

배포 상태(미배포/배포됨/운영 첫 적용 등)를 **설계 문서(spec·plan·tasks·설계문서·CLAUDE.md 등) 여러 곳에 서술**하기 전에도 `git log <prod-branch>..<integration-branch>` + 배포 메모리(예: [[deployment-live]])를 1회 교차확인한다. vault·이슈 노트의 배포 상태는 **그 시점 스냅샷**이라 그 사이 배포로 stale하다 — 이슈 노트를 근거로 여러 문서에 배포 서술을 복제하면 **오단정이 확산**되고 정정 비용이 문서 수만큼 커진다.

### 회피 절차

1. 배포 상태를 문서에 박기 직전: "이게 검증된 사실인가, 이슈 노트/기억 기반 추측인가" self-check.
2. `git log origin/<main>..origin/<develop>`로 무엇이 미배포 누적인지 확정 + 배포 메모리 line 대조.
3. 한 곳에 박기 전 확정 — 여러 문서에 복제한 뒤 정정하면 문서 수만큼 정정.

### 회귀 사례 — 2026-07-02 050 배포 상태 오단정

- stale한 vault ISSUE-054(6-28, 048 승격 전)를 근거로 "046/047 미배포 → V27~V29 운영 첫 적용"을 spec·plan·tasks·CLAUDE.md·설계문서 **6곳에 사실로 박음**. 실제는 048 main 승격(7-1)이 046/047/048을 이미 운영 배포(prod Flyway V30). 메모리 [[deployment-live]] line 21은 **이미 정확**했으나 active recall 실패. advisor 지적 → `git log origin/main..origin/develop`(049만)로 확정 → 6곳 정정.
- 회피 가능 시점: 첫 배포 서술을 문서에 박기 전 `git log main..develop` + 메모리 대조 1회. 회고 위 경로.
