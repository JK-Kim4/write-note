...
마무리는 **여러 SoT 를 동시에 정합**시키는 일이다 — git(develop 통합 / main 최종 배포) / vault(브랜치 무관 진척) / GitHub 이슈(트래커). 하나만 갱신하고 나머지를 잊으면 SoT 들이 서로 모순된다. 본 스킬은 세 곳을 한 번에 정합시키는 절차를 고정해, 사람이 매번 순서를 기억하지 않게 한다.
...
4. **main 승격은 별도 명시 승인** — develop 마무리(§절차 1~3)는 preview 까지다. production 배포(main 승격, §절차 4)는 사용자 명시 승인 없이 자동 진행 금지.
...
### 4. (선택) main 승격 — production 배포

브랜치 모델: **main = 최종 배포 브랜치**(`git push origin main` → Vercel production(soseolbi.com) 무중단 자동배포), **develop = 최종 배포 전 통합 브랜치**(push → preview). develop 마무리(1~3)는 preview 까지이며, 본 작업을 **지금 production 에 배포할지 사용자에게 명시적으로 묻는다.**

- 사용자가 승격을 원하지 않으면 여기서 종료 — develop(preview)까지만 반영.
- 사용자가 승격을 승인하면:

```bash
# 배포 순서 의존 선확인(HARD-GATE): FE/BE 방향 의존 있으면 CLAUDE.md §배포 순서 확인 후 진행
git checkout main
...
- **production 자동배포라 신중** — 사용자 명시 승인 없이 `git push origin main` 금지(§금지).
- **§19 한계:** prod 로그인 제약으로 인증 뒤 동작(authed)은 배포 후에도 검증 못 할 수 있다. build/test GREEN 을 authed 정합 증거로 단정하지 말고, 인증 화면 동작은 "미검증"으로 명시한다(CLAUDE.md §19).
...
- [ ] (main 승격 시) 사용자 명시 승인을 받았고, main 이 origin 과 동기이며 production 배포가 트리거됐는가
...
- **사용자 명시 승인 없이 `git push origin main`(= production 무중단 자동배포) — §절차 4 는 항상 묻고 진행**
