---
name: finish-work
description: 한 작업 단위(라운드·기능·버그픽스)가 GREEN 상태로 끝났을 때 통합·정리 마무리를 일괄 수행한다. ① 작업 브랜치를 develop 에 merge(--no-ff) + push + 머지된 브랜치 정리 ② sync-vault 스킬로 옵시디언 vault(02-PROGRESS·03-ISSUES) 갱신 ③ 관련 GitHub 이슈 일괄 갱신(완료 이슈 close + 완료 코멘트 + 마일스톤 진행률 확인)까지(= develop preview 통합). **production 배포(develop→main 승격)는 본 스킬 범위 밖이며 흐름에 포함하지 않는다 — 사용자가 별도로 "배포해줘"라고 명시 요청할 때만 분리 수행**한다. 사용자가 "작업 마무리", "마무리 작업 해줘", "끝내고 정리해", "merge하고 정리", "/finish-work" 라고 하거나 라운드/기능 구현이 게이트 GREEN 으로 끝난 직후 proactively suggest. **회고(retrospective) 와 구분** — 본 스킬은 통합·트래커 갱신 *메커니즘*이고, retrospective 는 5축 회고 *문서 작성*이다. 둘은 함께 쓸 수 있다(마무리 후 회고 제안). 출력 언어는 한국어(코드/명령어/고유명사 제외).
---
...
마무리는 **여러 SoT 를 동시에 정합**시키는 일이다 — git(develop 통합) / vault(브랜치 무관 진척) / GitHub 이슈(트래커). 하나만 갱신하고 나머지를 잊으면 SoT 들이 서로 모순된다. 본 스킬은 세 곳을 한 번에 정합시키는 절차를 고정해, 사람이 매번 순서를 기억하지 않게 한다. (production 배포는 본 스킬 범위 밖 — §배포는 분리.)
...
4. **배포는 본 스킬 범위 밖** — finish-work 는 develop 통합(preview, §절차 1~3)까지만 수행한다. production 배포(develop→main 승격)는 본 스킬 흐름에 **포함하지 않으며**, finish-work 도중 배포를 묻거나 유도하지 않는다. 배포는 사용자가 별도로 "배포해줘"라고 **명시 요청**할 때만 분리 단계로 수행한다(절차는 §배포는 분리 참조).
...
본 스킬의 절차는 여기까지다(develop preview 통합 완료). **production 배포는 아래 §배포는 분리 로 넘긴다 — finish-work 흐름에서 자동/유도 진행하지 않는다.**

## 배포는 분리 (본 스킬 범위 밖)

**finish-work 는 develop 통합(preview)까지다. production 배포(develop→main 승격)는 본 스킬 흐름에 포함하지 않는다.** 마무리 직후 "배포할까요?"로 묻거나 유도하지 말 것 — 배포는 사용자가 별도로 "배포해줘"라고 **명시 요청하는 독립 단계**다. finish-work 완료 보고 시 "배포는 별도 요청 시 진행" 정도만 안내한다.

사용자가 배포를 명시 요청하면 그때 아래를 별도 단계로 수행한다(배포 순서·§19 한계는 CLAUDE.md §배포 방식 준수):

```bash
# 배포 순서 의존 선확인(HARD-GATE): FE/BE 방향 의존 확인. BE 변경 있으면 OCI blue-green 선행(메모리 [[deployment-live]]).
git checkout main
git merge --no-ff develop -m "merge: develop → main — <한 줄 요약> · production 배포"
git push origin main      # → Vercel production 무중단 자동배포
git checkout develop      # 작업 위치 원복
```

- **production 자동배포라 신중** — 사용자 명시 요청 없이 `git push origin main` 금지.
- **§19 한계:** prod 로그인 제약으로 authed 동작은 배포 후에도 미검증일 수 있다 — "미검증"으로 명시(CLAUDE.md §19).
...
- [ ] 배포를 finish-work 흐름에 묶지 않았는가 (배포는 사용자 명시 요청 시 별도 단계)
...
- **finish-work 흐름에 배포를 묶거나 "배포할까요?"로 유도 — 배포는 본 스킬 범위 밖, 사용자 명시 요청 시에만 별도 수행**
- **사용자 명시 요청 없이 `git push origin main`(= production 무중단 자동배포)**
