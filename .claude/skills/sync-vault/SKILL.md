...
본 repo `CLAUDE.md` §"외부 SoT — 옵시디언 vault (HARD-GATE)" SoT.

- vault = **브랜치 무관 단일 진입점** — 본 repo `docs/plan/02-progress.md` 같은 브랜치 종속 문서가 여러 워크트리에서 conflict·누락되는 문제 우회
- Phase 단위 진척·이슈는 vault 에 박혀야 다른 워크트리·다음 세션의 Claude / 사용자가 일관 참조 가능
...
- PostToolUse hook (`.claude/settings.json` 의 `Bash(git push *)` matcher) 가 push 직후 `additionalContext` 로 본 스킬 호출 신호 + git context 박음
- 메인 Claude 는 다음 turn 에서 본 컨텍스트 보고 본 스킬 자동 실행
...
- 본 repo `CLAUDE.md` 의 "Claude 참조·갱신 의무" 표의 갱신 시점 정합
...
- 본 repo `CLAUDE.md` §"외부 SoT — 옵시디언 vault (HARD-GATE)"
- 본 repo `.claude/settings.json` `hooks.PostToolUse` (트리거)
- 본 repo `.claude/skills/retrospective/SKILL.md` (회고 → vault 입력 흐름)
