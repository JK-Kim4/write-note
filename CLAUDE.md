...
- [050 공유 페이지 고도화 (Share Enhancements) — 046/047(공유 링크·불변 스냅샷·공개 열람·작가 전용 위치 댓글·헤더 공유 메뉴; 048과 함께 운영 배포 완료 — prod Flyway V30, 2026-07-01 검증) 위 4갭 additive 해소. 브레인스토밍(ux-mockup 4종 승인). **US1(P1) 작가용 피드백 맥락 뷰**=한 공유 링크(스냅샷) 전문+받은 피드백 전부+반응 집계를 우측 패널로(안 B 확정; 호버·여백 말풍선 기각). 실측 근거: 공개열람 getSharedWork=본인댓글만+isActive 게이트, 작가조회 /api/projects/{id}/comments=projectId 단위 → 둘 다 부적합 → 신규 owner-scoped GET /api/share-links/{linkId}/works/{projectId}/feedback(비활성 링크도·그 스냅샷 전체 댓글·반응 묶음 단일 authz, 비소유 403). 진입=관리화면 링크별 "받은 피드백"(스냅샷 단위, 1:N 분리)·읽음=스냅샷 스코프(markReadBySnapshotId — unreadCommentCount가 스냅샷 단위라 projectId 읽음이면 같은 작품 타 링크 unread 0되는 버그 회피). **US2(P2) 비로그인 로그인**=공개 페이지 진입점(헤더 버튼·본문 CTA·드래그 유도)+로그인 페이지 이동(안 1, 모달 기각—카카오 어차피 전체 리다이렉트)+복귀=FE localStorage returnTo(/shared/ prefix 검증; 이메일 LoginForm→/entering·카카오 OAuth2SuccessHandler→홈 목적지 하드코딩이라 param 미지원→localStorage가 origin 왕복 생존, BE 0). **US3(P2) 이모지 반응 5종[❤️👍😮😢🔥] 공개 집계**(신규 share_reaction V31, UNIQUE(snapshot,anchor,emoji,reactor)로 토글 멱등, POST 추가/DELETE 제거, 집계는 공개 SharedWorkResponse.reactions embed[count+mine, 추가 fetch 0], 앵커=AnchorValidator 재사용 룰§32, emoji 서버 화이트리스트)+**전체 의견**(구간 미지정=share_comment 앵커 3필드 NOT NULL→nullable V32, 셋다null=전체·셋다값=구간·부분null=400, 작가 전용 유지). 반응만 공개·글댓글/전체의견 작가 전용(046 정책 유지). **US4(P3) 종이 위의 글**=공유 열람·작가 뷰를 흰 종이(--w-ms-page) 위+바깥 --w-ms-outer(집필화면 동일)+SharedReader 하드코딩색 #1f2937 토큰화→다크(FE only). 신규 에러코드 1(REACTION_EMOJI_INVALID 400; COMMENT_UNAUTHENTICATED 401·COMMENT_ANCHOR_INVALID 400·SHARE_FORBIDDEN 403 재사용, client.ts 신규 status 분기 0). 마이그레이션 V31(share_reaction)·V32(앵커 nullable). R1 BE(반응 도메인·앵커 nullable·작가 피드백 조회·스냅샷 읽음·공개응답 embed·IT)→R2 FE(종이 US4+로그인 US2, BE무관 조기 dogfooding)→R3 FE(작가 맥락뷰 US1)→R4 FE(반응 토글·전체 의견 US3). BE선행→FE후행. 046/047(V27~V29)·048(V30) 이미 운영 배포됨(develop은 049[마이그0]만 앞섬)→050 신규 마이그 V31/V32만 운영 첫 적용(라이브 share 스택 위 additive). 046/047 authed dogfooding 미수행(ISSUE-053)이라 050 로컬 검증이 share 첫 authed 확인 겸함(§15). 배포전 git log origin/main..origin/develop 재확인(룰§22). 데스크톱 폐기→미러 없음(web전용). TDD=BE 행위 IT·FE 순수(returnTo 검증)vitest·시각/앵커 DOM dogfooding(§14/§25/§19 authed 로컬 게이트). 범위밖=작가 답글/양방향 대화·리치텍스트·독자 댓글 수정·반응자 신원 공개. 시각 SoT=docs/research/2026-07-01-share-{author-feedback-view,guest-login,comment-enhancements,paper-layout}-mockup.html(승인). 설계 SoT=specs/050-share-enhancements/{spec,plan,research,data-model,contracts,quickstart}. 요구사항정의서=docs/superpowers/specs/2026-07-01-share-enhancements-design.md. constitution 빈템플릿→CLAUDE.md 룰 준용](specs/050-share-enhancements/plan.md)
- [049 공지사항 고정 슬롯 + 최신 슬롯 — 홈(메인) 상단 공지를 "고정 1건 + 최신 1건" 두 슬롯으로 구분 노출 + 고정을 붉은(테라코타) 강조로 식별. 핵심: 공지 도메인(030)에 isPinned·publishedAt·고정우선쿼리 이미 존재하나 메인에 배선된 적 없음(지금 고정 켜도 메인 효과 0, findFirst...IsPinnedDesc 미사용). 현재 배너=useLatestAnnouncement(list size1 content[0])=공개일 최신 1건만. 스키마·마이그레이션·신규 에러코드 0. clarify 1건(고정 다중 지정 처리=표시시점 1건 선택[display-pick]·어드민/BE 쓰기 무변경, 단일고정강제 기각). D1=전용 조회 GET /api/announcements/home→{pinned,latest} 서버측 pick+dedup(pinned=공개+고정 중 공개일 최신 1, latest=공개일 최신 1 단 pinned와 동일 id면 그다음; top-2 fetch로 dedup 충분). 요약 DTO에 isPinned 미추가(슬롯 이름으로 구분)·목록 endpoint 무변경(additive). 대안 기각=요약 isPinned+FE top-N pick(고정이 오래돼 top-N 밖일 수 있어 정확성 결함)·기존 findFirst 재사용(단일 배너용이라 둘 다 못줌). SecurityConfig 무변경(실측 L78 /api/announcements/* permitAll이 /home 커버, 리터럴 /home이 /{id}보다 우선매칭). 신규=repo findFirstByIsPublishedTrueAndIsPinnedTrueOrderByPublishedAtDesc()+service getHome()+HomeAnnouncementsResponse(pinned?,latest?)+GET /home+IT. FE=getHomeAnnouncements+useHomeAnnouncements(useLatestAnnouncement orphan 제거)+AnnouncementBanner 재구성(고정=앰버 채운카드+좌측 금빛바+채운「고정」배지+진한제목 / 최신=기존 teal pill 유지+인라인「공지」·세로적층·클릭→/notice/{id}·두 슬롯 고정 밝은배경+고정 어두운텍스트[teal-800/amber-900]로 라이트·다크 가독). FR-011=최신 배너 색 teal 유지·고정만 앰버 강조 추가(dogfooding으로 테라코타→순빨강→teal+amber 확정, 다크 제목 가독 버그 근본방지). 시각 SoT=docs/research/2026-07-01-announcement-pinned-latest-mockup.html(승인). 데스크톱 폐기→electron 미러 없음(web전용). R1 BE(선행)→R2 FE(후행), BE선행 이유=FE가 /home 호출. TDD=BE pick/dedup 행위 IT(both/고정만/최신만/없음/dedup/다중고정/미공개제외)·FE render vitest·시각 dogfooding(rule14, 라이트/다크/한글). dead code findFirst...IsPinnedDesc 대체됨(잔존, 제거는 컨펌). "말풍선"=확성기 배너(버블 도형 아님). /notice 페이지 색정합=범위밖. 설계 SoT=specs/049-announcement-pinned-latest/{spec,plan,research,data-model,contracts,quickstart}. constitution 빈템플릿→CLAUDE.md 룰 준용](specs/049-announcement-pinned-latest/plan.md)
