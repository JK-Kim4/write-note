...
사용자 결정 = **룰 반영 안 함.** 회고 스킬을 "앱 구현 종속 교훈은 룰로 승격 말고 포괄적 원칙만"으로 보강(§5-2). 두 후보의 처리:
- **measure↔render 줄바꿈 정합(U+2028)** — 앱 구현 종속(특정 문자·특정 렌더 경로)이라 룰 부적합 → **§3 작업별 교훈으로 격하**(다음 자체엔진 작업 시 점검 항목). 일반 명제로 추상화는 가능하나("측정과 표시가 다른 메커니즘에 의존하면 정합이 깨진다") 룰화하지 않음.
- **health/집계 신호 단정 금지** — 비교적 일반적이나, 기존 §11(관찰→확정)·금지2(단정 금지)에 이미 포섭됨 → 별도 룰 추가 불필요.
