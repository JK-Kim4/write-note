...
**Goal:** 비로그인 방문자가 루트 `/`에서 소설비 소개를 보고 가입/로그인으로 진입하게 한다(기존 A형 대시보드는 `/home`으로 이전).
...
- 에셋 재사용: `/soseolbi-logo.png`(투명 와이드), `/soseolbi-mark.png`(투명 정사각) — 이미 `frontend/public/`에 존재.
...
```css
/* 소설비 공개 랜딩 — 고정 웜 팔레트(앱 .dark 테마와 무관). */
.landing {
...
.landing-logo { width: 132px; height: 40px; background: url('/soseolbi-logo.png') left center / contain no-repeat; }
...
.landing-fmark { width: 110px; height: 30px; background: url('/soseolbi-mark.png') left center / contain no-repeat; margin-bottom: 10px; }
