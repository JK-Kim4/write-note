...
| DB | **PostgreSQL** (OCI Compute self-managed) | Supabase → OCI 전환 (2026-06-16) | V1 런칭 인프라 OCI 확정 (pgvector 확장 V2 대비) |
...
| 백엔드 호스팅 | **OCI Compute 인스턴스** (self-managed, Docker 또는 JAR+systemd) | Render → OCI 전환 (2026-06-16) | V1 런칭 인프라 OCI 확정 |
...
| 단계 | 인프라 | 등록 비용 |
|---|---|---|
| V1 (웹 + PWA) | Vercel 무료 + OCI Compute (Always Free) + OCI self-managed PostgreSQL | $0 (OCI Always Free 범위) |
| V2 (앱스토어 진입) | 동일 | + Apple Developer 연 $99 + Google Play 1회 $25 = 첫 해 $124, 이후 $99/년 |

### 2-3. 호스팅 운영 주의 (OCI Compute self-managed, 2026-06-16 전환)

- **self-managed 직접 책임** — Render PaaS와 달리 VM·OS·방화벽(VCN Security List/NSG + OS firewalld/iptables)·리버스 프록시(nginx/Caddy)·TLS(Let's Encrypt 자동 아님)·프로세스 자동재시작(systemd)·DB 백업(`pg_dump` cron)을 직접 구성·운영
- **Always Free 범위** — Compute Always Free 한도(예: ARM Ampere A1 또는 AMD micro) 내 운영 목표. 한도 초과 시 과금 — 리소스 사양 확인 후 프로비저닝
- **DB = 같은 Compute에 self-managed PostgreSQL** — public 미노출(로컬 소켓 또는 같은 VCN 사설 접근), 백엔드와 동일 인스턴스 또는 같은 VCN 분리
- **배포** — Render의 push-자동배포 대신 수동 배포 또는 별도 CI(GitHub Actions → OCI) 구성 (V1은 수동 가능)
...
| 본질 제약 | 현 결정 | 정합성 |
|---|---|---|
| $0/월 (DESIGN.md 163줄) | Vercel 무료 + OCI Compute Always Free + OCI self-managed PostgreSQL | ✅ (Always Free 범위) |
...
| 2026-06-16 | **운영 인프라 OCI 전환.** 백엔드 호스팅 Render → OCI Compute(self-managed), DB Supabase Postgres → OCI Compute self-managed PostgreSQL. 프론트 Vercel 유지. §2-3 운영 주의를 OCI self-managed 기준으로 교체. | V1 런칭 인프라 OCI 확정 (사용자 결정). 상세 = `04-web-launch-v1-plan.md` Round 4 |
