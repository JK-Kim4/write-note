# 핸드오프 — Scope B 후속: 대시보드 허브 (브레인스토밍 진행 중) + 017 집필실 3단 완료
...
### 2-3. 멈춘 지점 — **사용자에게 던진 질문(미응답)**
목업(`dashboard-reentry-hub.html`)을 보여주고 **내용 방향**을 물었음:
- **A. 재진입 허브** — 인사+날짜 / 이어서 쓰기(최대 타일) / 작품 빠른진입 / 최근 곁쪽지. streak·목표·그래프·자동인용구 제외. 백엔드 0. **(목업이 보여주는 추천안)**
- **B. 절충** — A + "이번 주 집필 시간" 한 줄(기존 work_session 데이터, 목표/streak/그래프 없음). 백엔드 0~최소.
- **C. 더 손보기** — 배치·구성 변경.

**→ 다음 세션 첫 액션: 이 A/B/C 응답을 받아 내용 확정 후 brainstorming 계속.**
...
1. 본 핸드오프 + `PRODUCT.md` + 목업 HTML 확인.
2. **대시보드 내용 방향 A/B/C 사용자 응답 받기**(§2-3) → brainstorming 계속(디자인 시스템·데이터 매핑·`/`→`/library` 영향).
3. design doc `docs/superpowers/specs/2026-06-1X-dashboard-hub-design.md` 작성 → 사용자 리뷰.
