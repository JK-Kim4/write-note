...
| 5 | `05-memo-capture-inbox/` | 빠른 메모 캡처와 inbox 구현 | ✅ 완료 (2026-06-05, dogfooding 통과, `007-phase-5-memo-capture-inbox` develop merge 대기) — 캡처(모달+인라인)+자동연결+전체/미연결 필터+soft delete(`deleted_at` v3)+되돌리기 토스트. SDD 풀파이프(specify→plan→tasks→analyze→implement). TDD 14 files 75 tests + tsc + build GREEN, 회귀 0. dogfooding 발견: `MemoPanel`(집필창 연결 메모 패널)은 더미 → Phase 6 결선. spec: `specs/007-phase-5-memo-capture-inbox/` |
