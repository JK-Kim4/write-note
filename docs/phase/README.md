...
| 5 | `05-memo-capture-inbox/` | 빠른 메모 캡처와 inbox 구현 | 🟡 구현 자동화 GREEN (2026-06-05, `007-phase-5-memo-capture-inbox` 미merge) — 캡처(모달+인라인)+자동연결+전체/미연결 필터+soft delete(`deleted_at` v3)+되돌리기 토스트. SDD 풀파이프(specify→plan→tasks→analyze→implement). TDD 14 files 75 tests + tsc + build GREEN, 회귀 0. dogfooding 대기. spec: `specs/007-phase-5-memo-capture-inbox/` |
